...
export default function ClientLogos() {
  return (
    <section className="py-20" aria-labelledby="clients-heading">
      <div className="mx-auto max-w-7xl px-6">
        <p id="clients-heading" className="mb-10 text-center text-xs uppercase tracking-[0.4em] text-white/50">Trusted By</p>
            ...
            {[...clients, ...clients].map((client, index) => {
              ...
              return (
                <div key={`${client.name}-${index}`} aria-hidden={isDuplicate} className="flex h-20 w-52 items-center justify-center rounded-2xl border border-white/10 bg-white/5 px-6 backdrop-blur-sm transition-all duration-300 hover:border-white/20 hover:bg-white/10">
                  <Image src={client.logo} alt={isDuplicate ? "" : client.name} width={140} height={40} className="h-10 w-auto object-contain" />
                </div>
              );
            })}
          </div>
